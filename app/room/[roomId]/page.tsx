...
export default function RoomPage() {
  ...
  const removedUids = useRef<string[]>([])
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => { const body = isChat ? { ownerUid: ownUid, users: users.filter(user => user.ownerUid === ownUid), removeUids: removedUids.current } : { content: users[0]?.text || '' }; if (!isChat) setSyncState('syncing'); try { const response = await fetch(`/api/room/${roomId}${isChat ? '?mode=chat' : ''}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) }); const data = await response.json(); if (!isChat && typeof data.version === 'number') localVersion.current = data.version; setSyncState('synced'); dirty.current = false; removedUids.current = [] } catch { /* retry on next local edit */ } }, 800)
  ...
  const removeUser = (uid: string) => { removedUids.current.push(uid); dirty.current = true; setUsers(value => value.filter(user => user.uid !== uid)) }
